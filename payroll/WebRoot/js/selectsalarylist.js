layui.define(['laypage', 'layer', 'form', 'pagesize'], function (exports) {
  ...
  //自定义验证
    ...
    //监听搜索的表单提交
    form.on('submit(salaryformSearch)', function (data) {
        var index = layer.load(1);
        var data1 = new Array();
        setTimeout(function () {
            //模拟登陆
            ...
            $.ajax({
        		type: "POST",	//提交方式
        		url: "search_salary_byname.form",	//提交的路径
        		data: {name:data.field.keywords_name},
        		dataType: "json", //返回数据的类型
        		async: false, //设置同步请求
        		success: function(result){ //成功之后的回调函数
    				for(var i in result){//遍历json数组时，这么写i为索引，0,1
       				 ...
       				 data1.push({ id: result[i].id, number: result[i].number, name: result[i].name, deptName: result[i].deptName, level: result[i].level, salary: result[i].salary.base, subsidy: result[i].subsidy.subsidys, winning: prizes, check_job: result[i].check_job.checks});
    				}
        		}
        	});
        ...
        setTimeout(function () {
            ...
            for (var i = 0; i < data1.length; i++) {
                ...
                html += "<tr>";
                html += "<td>" + item1.id + "</td>";
                html += "<td>" + item1.number + "</td>";
                html += "<td>" + item1.name + "</td>";
                html += "<td>" + item1.deptName + "</td>";
                html += "<td>" + item1.level + "</td>";
                html += "<td>" + item1.salary + "</td>";
                html += "<td>" + item1.subsidy + "</td>";
                html += "<td>" + item1.winning + "</td>";
                html += "<td>" + item1.check_job + "</td>";
                html += "<td>" + (item1.salary + item1.winning + item1.subsidy) + "</td>";
                html += "<td>" + (item1.salary + item1.winning + item1.subsidy + item1.check_job ) + "</td>";
                html += "</tr>";
            }
            html += '</tbody>';
            html += '</table>';
    ...
    exports('selectsalarylist', selectsalarylist);
});